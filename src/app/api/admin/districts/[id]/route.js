...
export async function PATCH(req, context) {
  ...
  const idValidation = validateUuid(id, "District id");
  ...
  try {
    ...
    return NextResponse.json({ error: "Unable to update district" }, { status: 500 });
  }
}
...
export async function DELETE(_req, context) {
  ...
  try {
    await prisma.district.delete({ where: { id: idValidation.value } });
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      if (error.code === "P2003") {
        return NextResponse.json(
          { error: "Remove areas linked to this district before deleting" },
          { status: 400 }
        );
      }
    }

    return NextResponse.json({ error: "Unable to delete district" }, { status: 500 });
  }
}
