...
export async function PATCH(req,context) {
  ...
  const idValidation = validateUuid(id, "State id");
...
export async function DELETE(_req, context) {
  ...
  try {
    await prisma.state.delete({ where: { id: idValidation.value } });
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      if (error.code === "P2003") {
        return NextResponse.json(
          { error: "Remove districts linked to this state before deleting" },
          { status: 400 }
        );
      }
    }

    return NextResponse.json({ error: "Unable to delete state" }, { status: 500 });
  }
}
